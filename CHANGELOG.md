...
commands; the file bytes travel on a plain TCP socket rather than the bus, because a whole-file
sequential read gains nothing from bus framing and the device CPU is the ceiling. The contract is
[`docs/protocol/recordings_pull.md`](docs/protocol/recordings_pull.md).

Schema (all additive — new optional fields and new oneof arms):

- `ListRecordings.cursor` (2) and `.session_name` (3): paging, and one session with its files.
- `RecordingEntry.active` (17), `.file_count` (18), `.files` (19); new `RecordingFile` with
  `name`, `size`, `mtime_ns`, `writing` (open for write: listed, not pullable), `complete`,
  `encrypted`.
- `RecordingsList.next_cursor` (2) and `.read_while_recording` (3).
- `Command.open_recording_file` (43) → `CommandResult.file_open` (13), a `RecordingFileOpen`
  (`port`, `offset`, `length`, `file_size`, `mtime_ns`).
- `Command.delete_recording` (44).
- `nanopb.options`: every new inbound name and the cursor at `max_size:64`, pinned equal by
  `test_nanopb_options.py`; the listing's new lists and strings are `FT_POINTER`.

Clients:

- Python `visio_schema.wire.recordings`, C++ `visio_schema/wire/recordings.hpp`, TypeScript
  `@gilabs/visio-schema/wire/recordings` (new `exports` subpath): builders, decoders, paging,
  and `pull_file` / `PullFile` / `pullFile` — open, receive, and on a short read open again at the
  bytes received with the file's size and mtime pinned. `PULL_QUIESCE_RULES` /
  `kPullQuiesceRules` is the stream policy a host holds on its own link for the pull (§7).
- `tests/golden/recordings_wire_vectors.txt` (generated by `scripts/gen_recordings_wire_vectors.py`)
  pins the three codecs to each other.

Pull and delete work over any direct IP link (USB-NCM or Wi-Fi); `forbidden_link` is for a link
that is not the host's own (a hub's leaf link, an unknown endpoint); the byte socket listens on all
the device's addresses.

No facade change in either language.
