...
| **C. Wholly Visio** | `proto/visio/*` | Visio-native name (e.g. `visio.sensor.v1.ImuRaw`) | not natively; use Plot panel for scalar fields, or write a converter extension |
...
|---|---|---|
| `visio.sensor.v1.ImuRaw` | `STREAM_IMU_RAW` | Foxglove has NO raw-IMU schema (verified against `foxglove-sdk` v0.24.0). Bundled per-IMU samples preserve per-tick fidelity that UMI's grid-ZOH discarded. |
| `visio.sensor.v1.EncoderRaw` | `STREAM_ENCODER_RAW` | Raw count, magnetic-field magnitude, AGC, magnet-detection enum — diagnostics for bring-up. Foxglove has nothing like it; `foxglove.JointState` only covers cooked position/velocity/effort. |
| `visio.sensor.v1.SystemHealth` | `STREAM_SYSTEM_HEALTH` | CPU / RAM / battery / disk / stream-clients periodic telemetry. No Foxglove equivalent. |
| `visio.sensor.v1.AudioCompressed` | `STREAM_AUDIO_COMPRESSED` | `foxglove.RawAudio` only supports PCM-S16. We need AAC-LC ADTS / Opus / Vorbis for the UMI headset mic path. |
| `visio.sensor.v1.ButtonEvent` | `STREAM_BUTTON_EVENT` | Named-button edge events; no Foxglove equivalent. |
| `visio.input.v1.QuestControllerState` | `STREAM_CONTROLLER_STATE` | Quest controller buttons + sticks for BOTH hands per tick. No Foxglove equivalent (`foxglove.JointStates` is the closest but joints are continuous, not buttons). |
| `visio.control.v1.Command` | `STREAM_COMMAND` | Host→device intents (StartRecording, StopRecording, Identify). Application-specific. |
| `visio.geometry.v1.Twist` | `STREAM_TWIST` | 6-DoF velocity. Foxglove only has linear `Velocity3`; no Twist. We use `foxglove.Vector3` primitives so it composes. |
| `visio.service.timesync.v1.{Request,Response}` | `STREAM_TIMESYNC` | NTP-style exchange; bus-layer concern, no analogue. |
| `visio.service.device_info.v1.DeviceInfo` | `STREAM_DEVICE_INFO` | Boot-time identity + stream-capability declaration. |
| `visio.service.heartbeat.v1.Heartbeat` | `STREAM_HEARTBEAT` | Liveness + backpressure hint. |
| `visio.service.schema.v1.{Request,Response}` | `STREAM_SCHEMA_QUERY` | Runtime descriptor lookup. |
...
3. Bump `visio-schema` minor version. Any peer using
   `visio.wire.v1.Header.stream` enum values that map to foxglove
